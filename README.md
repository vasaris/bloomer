...
Сделано по **Sprint 6** (`v0.6.0`): каркас + планировщик (тихие часы, снуз), M0 адаптация + астма-чек Макса, геймификация (стрики 🚶/👃/🧠, XP, уровни, ачивки), M3 груминг, M5 тренинг/трюфели, M4 здоровье (прививки/обработки/вет, журнал веса), **M6 социализация** (чек-лист «новый опыт» с прогресс-баром, идея социализации дня) и **M7 путешествия** (проверка жары перед выездом через погодное API, чек-лист сборов в тур, ачивка «Путешественник»). Утренний бриф теперь предупреждает о жаре. Бот деплоится и шлёт пуши по расписанию.
...
- ✅ **Sprint 1–6** — адаптация+астма, геймификация, груминг, тренинг/трюфели, здоровье+вес, **социализация+путешествия** (сделано).
- **Sprint 7** — Claude API (`/ask` с системным промптом из `BLUMER_SYSTEM.md`).
- **Sprint 8** — полировка: недельные обзоры, экспорт журнала, бэкапы, настройки времени пушей.
